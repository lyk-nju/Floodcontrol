...
class T2MEmbedder:
    ...
    def __call__(self, features: torch.Tensor) -> torch.Tensor:
        ...
        movement = self.movement(normalized[None, :, :-4])
        movement_length = features.shape[0] // 4
        embedding = self.motion(
            movement, torch.tensor([movement_length], device=self.device)
        )
...
def main() -> None:
    ...
    for name in names:
        ...
        exact_reference_embeddings.append(embedder(exact_reference))
        exact_converted_embeddings.append(embedder(exact))
        approximate_reference_embeddings.append(embedder(source))
        approximate_converted_embeddings.append(embedder(approximate))
        processed += 1
    ...
    exact_reference = torch.stack(exact_reference_embeddings)
    exact_converted = torch.stack(exact_converted_embeddings)
    approximate_reference = torch.stack(approximate_reference_embeddings)
    approximate_converted = torch.stack(approximate_converted_embeddings)
    result = {
        "samples": processed,
        "exact_drop": {
            "features": exact_error.result(),
            "embedding_l2_mean": float((exact_reference - exact_converted).norm(dim=-1).mean()),
            "embedding_cosine_mean": float(torch.nn.functional.cosine_similarity(exact_reference, exact_converted).mean()),
            "fid": _fid(exact_reference_embeddings, exact_converted_embeddings),
        },
        "approximate_tail": {
            "features": approximate_error.result(),
            "embedding_l2_mean": float((approximate_reference - approximate_converted).norm(dim=-1).mean()),
            "embedding_cosine_mean": float(torch.nn.functional.cosine_similarity(approximate_reference, approximate_converted).mean()),
            "fid": _fid(approximate_reference_embeddings, approximate_converted_embeddings),
        },
        "drop_against_full_length": {
            "embedding_l2_mean": float(
                (approximate_reference - exact_converted).norm(dim=-1).mean()
            ),
            "embedding_cosine_mean": float(
                torch.nn.functional.cosine_similarity(
                    approximate_reference, exact_converted
                ).mean()
            ),
            "fid": _fid(
