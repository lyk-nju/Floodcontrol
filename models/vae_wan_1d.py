...
class BodyVAE(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if motion_stats_path is not None:
            stats = VAEStatistics.load(motion_stats_path, expected_fps=self.fps)
        ...
        if latent_stats_path is not None:
            ...
            if metadata.get("converter_version") != MOTION_CONVERTER_VERSION:
                raise ValueError("latent statistics converter version mismatch")
            if metadata.get("weights_kind") != "ema":
                raise ValueError("latent statistics must be generated by EMA tokenizer weights")
    ...
    def encoder_context_tokens(self) -> int:
        """Exact causal warm-up prefix required by online body encoding."""
    ...
    def decode(
        ...
        if frame_valid_mask is not None:
            ...
            contact_logits = torch.where(
                ...
                output.contact_logits.new_full((), -20.0),
            )
        else:
