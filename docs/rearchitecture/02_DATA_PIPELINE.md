...
`tools/compute_ldf_root_stats.py`复用同一scaled-ARDY分布：自然parent最多50 tokens；真实序列前缀均匀采样`H∈[0,N-5]`，中间parent均匀采样`H∈[1,N-5]`。`H>0`取第`4H-1`帧作为anchor，合法`H=0`取序列第0帧。产物仍只保存`root_mean/root_std [5]`。HumanML-only与HumanML+BABEL训练统一复用这份HumanML canonical root statistics。
...
LDFSpanBatch
  per-sample natural parent window capped at 50 tokens + right padding
