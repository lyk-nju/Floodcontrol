...
状态：`MODEL_CORE_IMPLEMENTED`。公共合同、Root/Body主干、CFG和Hybrid流式状态已经落地；strict-4 VAE、真实数据、训练与Web runtime仍待后续接线。具体正式网络规模、loss权重和训练schedule仍由实验决定。
...
- 真实训练与Web入口在strict-4 VAE/data/runtime接入前明确fail-fast。
...
8. 接入 strict-4 causal body VAE 与 commit-time decoder transaction。
...
训练与Web入口同步改为`BLOCKED_ON_STRICT4_VAE`，因此不会回退到旧VAE或留下伪兼容路径。后续恢复端到端能力时直接围绕新合同建设。
