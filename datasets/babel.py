...
class BABELDataset(Dataset):
    """Load complete BABEL samples without training-time transformations."""
    ...
    def _load_file_list(self) -> list[dict[str, object]]:
        """Build a lightweight index from BABEL split TXT files."""
        ...
        for meta_path in self.file_list:
            if not meta_path.is_file():
                raise RuntimeError(f"BABEL split file not found at {meta_path}")
            ...
            for line_number, line in enumerate(
                ...
                if not motion_file.is_file():
                    raise RuntimeError(f"BABEL motion file not found at {motion_file}")
                ...
                if text_root is not None:
                    ...
                    if not text_file.is_file():
                        raise RuntimeError(f"BABEL text file not found at {text_file}")
                    data["text_path"] = text_file

                dataset.append(data)

        if len(dataset) == 0:
            raise RuntimeError("BABEL split contains no samples")
        return dataset
    ...
    def load_text(
        ...
        """Load segmented BABEL captions as frame intervals."""
        ...
        for line_number, line in enumerate(path.read_text().splitlines(), start=1):
            ...
            if len(line_split) != 4:
                raise ValueError(f"invalid BABEL text row at {path}:{line_number}")
